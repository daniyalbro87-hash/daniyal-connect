        ...
        {messages.map((m, i) => {
          ...
          return (
            <div key={m.id} className={`flex ${mine ? "justify-end" : "justify-start"} animate-fade-up`}>
              <div className={`max-w-[80%] ${showTail ? "mt-2" : "mt-0.5"} ${hasText ? "px-4 py-2.5" : "p-1.5"} rounded-3xl shadow-soft
                ${mine
                ...
                {hasText && <div className="whitespace-pre-wrap break-words text-[15px] leading-relaxed">{m.text}</div>}
                {!hasText && (
                  <div className={mine ? "" : ""}>
                    <MessageMedia msg={m} onOpenImage={(urls, index) => setViewer({ urls, index })} />
                    {m.text && <div className="px-2 pt-2 pb-1 text-[14px]">{m.text}</div>}
                  </div>
                )}
                <div className={`text-[10px] mt-1 flex items-center gap-1 justify-end ${mine ? "text-white/70" : "text-muted-foreground"} ${hasText ? "" : "px-2 pb-1"}`}>
