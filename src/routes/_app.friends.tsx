...
function FriendsPage() {
  ...
  const onDecline = async (r: ReqRow) => {
    if (!user) return;
    setBusy(r.id);
    try { await declineFriendRequest(user.uid, r.from); } finally { setBusy(null); }
  };
  ...
  return (
    <div className="min-h-[100dvh] mx-auto w-full max-w-2xl px-4 pt-6 pb-28 overflow-x-hidden">
      ...
      <section className="mb-6">
        <div className="text-xs uppercase tracking-wide text-muted-foreground px-2 mb-2">Incoming ({incoming.length})</div>
        {incoming.length === 0 ? (
          <div className="glass rounded-3xl p-6 text-center text-sm text-muted-foreground shadow-soft">No pending requests.</div>
        ) : (
          <div className="space-y-2">
            {incoming.map((r) => (
              <div key={r.id} className="flex items-center gap-3 p-3 rounded-2xl bg-surface shadow-soft animate-fade-up">
                {r.fromProfile?.photoURL ? (
                  <img src={r.fromProfile.photoURL} alt="" className="w-12 h-12 rounded-2xl object-cover" />
                ) : (
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-semibold truncate">{r.fromProfile?.displayName || "User"}</div>
                  <div className="text-xs text-muted-foreground truncate">@{r.fromProfile?.username || "unknown"}</div>
                </div>
                <button onClick={() => onDecline(r)} disabled={busy === r.id} className="px-3 py-2 rounded-xl bg-muted text-sm font-semibold disabled:opacity-60">Decline</button>
                <button onClick={() => onAccept(r)} disabled={busy === r.id} className="px-3 py-2 rounded-xl gradient-brand text-white text-sm font-semibold shadow-glow disabled:opacity-60">Accept</button>
              </div>
            ))}
          </div>
        )}
      ...
      <section>
        <div className="text-xs uppercase tracking-wide text-muted-foreground px-2 mb-2">Sent ({outgoing.length})</div>
        {outgoing.length === 0 ? (
          <div className="glass rounded-3xl p-6 text-center text-sm text-muted-foreground shadow-soft">You haven't sent any requests.</div>
        ) : (
          <div className="space-y-2">
            {outgoing.map((r) => (
              <div key={r.id} className="flex items-center gap-3 p-3 rounded-2xl bg-surface shadow-soft">
                <div className="w-12 h-12 rounded-2xl bg-muted grid place-items-center text-muted-foreground">…</div>
                <div className="flex-1 min-w-0">
